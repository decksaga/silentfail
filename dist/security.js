...
// ─── Scanner ───
function scanText(text, patterns, category, severity, server, tool, recommendation) {
    const findings = [];
...
function scanToolSchema(server, toolName, schema) {
    const findings = [];
...
export function runSecurityScan(servers) {
    const findings = [];
    let scannedTools = 0;
    for (const server of servers) {
        // ─── Check server command & args ───
        const cmdLine = `${server.config.command} ${(server.config.args ?? []).join(" ")}`;
        findings.push(...scanText(cmdLine, DANGEROUS_COMMAND_PATTERNS, "dangerous_command", "high", server.name, undefined, "Review the server's startup command. Ensure it comes from a trusted source."));
        ...
        for (const tool of server.tools) {
            ...
            // Prompt injection
            findings.push(...scanText(textToScan, PROMPT_INJECTION_PATTERNS, "prompt_injection", "critical", server.name, tool.name, "This tool's description may contain prompt injection. Remove or replace this server immediately."));
            // Exfiltration
            findings.push(...scanText(textToScan, EXFILTRATION_PATTERNS, "exfiltration", "critical", server.name, tool.name, "This tool may attempt to exfiltrate data. Inspect the tool's source code and remove if untrusted."));
            // Dangerous commands in descriptions
            findings.push(...scanText(textToScan, DANGEROUS_COMMAND_PATTERNS, "dangerous_command", "high", server.name, tool.name, "Tool description references dangerous operations. Review the server source code."));
            // Encoded payloads
            findings.push(...scanText(textToScan, ENCODED_PAYLOAD_PATTERNS, "encoded_payload", "high", server.name, tool.name, "Tool schema contains encoded data that may hide malicious instructions. Decode and inspect manually."));
            // Schema analysis
