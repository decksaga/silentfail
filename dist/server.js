...
- Scan duration: ${report.scanDurationMs}ms`);
        ...
        for (const tool of s.tools) {
            ...
            if (test) {
                ...
                if (test.status === "error" && test.error)
                    badge += `: ${test.error.slice(0, 100)}`;
                if (test.status === "ok" && test.detail)
                    badge += ` (${test.detail})`;
            }
    ...
    // ── Token budget
    if (ok.length > 0) {
        const budgetLines = ["## TOKEN BUDGET"];
    ...
    if (report.conflicts.length > 0) {
        const conflictLines = ["## ⚠️ CONFLICTS", "These tools exist in multiple servers — the model might call the wrong one:"];
    ...
    if (report.security) {
        ...
        const riskIcons = { clean: "✅", low: "🟢", medium: "🟡", high: "🟠", critical: "🔴" };
        const secLines = [
            "## 🛡️ SECURITY",
    ...
    if (report.recommendations.length > 0) {
        ...
        const iconMap = { critical: "🔴", warning: "🟡", info: "💬", ok: "✅" };
        for (const rec of report.recommendations) {
            recLines.push(`- ${iconMap[rec.type] ?? ""} **[${rec.server}]** ${rec.message}`);
            if (rec.action)
                recLines.push(`  → ${rec.action}`);
        }
...
async function lazyScan(sources, testTools) {
    const { runScan } = await import("./scanner.js");
    return runScan(sources, { testTools });
}
async function lazyDashboard(report) {
...
server.tool("scan_mcp_servers", `ALWAYS call this tool when the user asks to scan, check, diagnose, or analyze MCP servers — in any language.
...
server.tool("open_silentfail_dashboard", `Open the SilentFail visual web dashboard in the user's browser.
