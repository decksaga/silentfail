...
async function testTool(client, tool) {
    ...
    try {
        ...
        const result = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("Tool call timeout (8s)")), 8000)),
        ]);
...
function generateRecommendations(servers) {
    const recs = [];
...
async function scanServer(name, config, source, options) {
    const start = Date.now();
    ...
    try {
        ...
        await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("Connection timeout")), options.timeoutMs)),
        ]);
...
function findConflicts(servers) {
...
export async function runScan(sources, options = {}) {
    const start = Date.now();
    const servers = [];
    ...
    for (const source of sources) {
        for (const [name, config] of Object.entries(source.servers)) {
            ...
            if (name === "silentfail" || argsStr.includes("silentfail/dist/server"))
                continue;
